 ...
 */
class InstalledAppActionsSpeedMenu(actionPresenter: AppActionsContract.Presenter) : SpeedDialMenuAdapter() {
    ...
    override fun getMenuItem(context: Context, position: Int): SpeedDialMenuItem = when (position) {
        ...
        AppActions.BUILD_INFO.ordinal -> SpeedDialMenuItem(context, R.drawable.ic_allow, context.getString(R.string.show_app_system_page))
    ...
    override fun onMenuItemClick(position: Int): Boolean = presenterRef.get()?.let { presenter ->
        when (position) {
            ...
            AppActions.BUILD_INFO.ordinal -> presenter.showSystemPageClick()
